# pipeline_agent_json.py - Updated to use JSON database
# =============================================================================
# PIPELINE AGENT - ANTEATAL GUIDELINES QUERY MODULE 
# =============================================================================
# VERSION: 7.0
# AUTHOR: Clinical Decision Support Team 5
# CREATED: 13/03/2026
...
#
# DATA SOURCE:
#   - augment_db4.json: 37 antenatal conditions extracted from PDF guidelines
...
#   print(result["sections"]["clarify"])  # Clarification questions
#   print(result["sections"]["tests"][0]["guideline_reference"])  # Guideline ref
#
# SUPPORT:
...
def query_guidelines(
    ...
    Returns:
        Dict[str, Any]: Structured guideline response with the following schema:
        {
            ...
            "sections": {
                ...
                "clarify": List[Dict],    # Clarification questions
    ...
    Example:
        ...
        >>> print(result["sections"]["clarify"])
        [{'question': 'Does the patient meet diagnostic criteria...?', ...}]
...
def generate_json_response(
    ...
    with categorized sections (actions, tests, ultrasound, clarify, etc.).
    Each claim includes a guideline_reference to the source document.
    ...
    Returns:
        Dict[str, Any]: Standardized response schema with clarify section
    ...
    Response Schema:
        {
            ...
            "sections": {
                "guidelines": [...],
                "actions": [...],         # Each with guideline_reference
                "tests": [...],           # Each with guideline_reference
                "ultrasound": [...],      # Each with guideline_reference
                "follow_up": [...],       # Each with guideline_reference
                "clarify": [...],         # Each with guideline_reference
                "decision_points": [...], # Each with guideline_reference
                "plan": {...}
            },
            "personalization": {...}
        }
    ...
    for arg in item.get("arguments", []):
        ...
        guideline_ref = arg.get("guideline_reference", source_ref)  # Get guideline reference
        
        # Create entry with guideline reference
        entry = {
            "claim": claim,
            "timing": timing if timing and timing != "As per guideline" else None,
            "category": category,
            "source_column": source_column,
            "guideline_reference": guideline_ref  # Include guideline reference
        }
        ...
        # Extract clarification questions from conditional claims
    ...
    # Add criteria-based clarification (always first)
    if criteria:
    ...
    # Build response structure WITH clarify section
    response = {
        "version": "7.0",
        ...
        "sections": {
            ...
            "clarify": clarify_items if clarify_items else [],  # Clarify section included
    ...
    # In generate_json_response(), update the personalization section:

    if patient_context:
        bmi_cat = categorize_bmi(patient_context.get("bmi"))
    
        # Map PDF-faithful categories to risk flags
        age_risk = patient_context.get("age", 0) >= 40
        bmi_risk = bmi_cat in ["bmi_35_39.9", "bmi_over_40", "low_bmi"]
        vte_risk = any(kw in " ".join(patient_context.get("medical_history", [])).lower() 
                  for kw in ["dvt", "thrombosis", "clot"])
    
        response["personalization"] = {
            "age_risk": age_risk,
            "bmi_category": bmi_cat,  # Now returns PDF-faithful values
            "bmi_risk": bmi_risk,     # NEW: Simple risk flag
            "vte_risk": vte_risk,
            "gestational_age": patient_context.get("gestational_age_weeks"),
            "triggered_conditions": get_triggered_conditions(patient_context)
        }
...
# =============================================================================
# HELPER: CONVERT CLAIM TO CLARIFY QUESTION
# =============================================================================
...
def _claim_to_clarify_question(claim: str) -> Optional[str]:
    ...
    This helper function extracts the condition portion from clinical claims
    and formats them as yes/no questions for clinical verification.
    
    Args:
        claim (str): Original claim text from argument database
    
    Returns:
        Optional[str]: Clarification question, or None if not applicable
    
    Examples:
        >>> _claim_to_clarify_question("If anaemia start oral iron")
        "Is anaemia?"
        
        >>> _claim_to_clarify_question("unless known haemoglobinopathy")
        "Is known haemoglobinopathy NOT present?"
        
        >>> _claim_to_clarify_question("if Hb < 70g/l or if symptomatic")
        "Is Hb < 70g/l or if symptomatic?"
    """
    claim_lower = claim.lower()
    
    # Extract condition from IF clauses
    ...
    # Extract condition from UNLESS clauses
    ...
    # Extract condition from KNOWN/HISTORY clauses
    ...
    # Extract condition from TOLERATING/INDICATED clauses
...
def categorize_bmi(bmi: float) -> str:
    ...
    """
    #We will use the values provided in the pdf only.
    """
    Categorize BMI based ONLY on PDF-defined thresholds.
    
    Categories from ANTENATAL CARE SCHEDULE.pdf:
        - low_bmi: < 18.5 (condition: Low BMI < 18.5)
        - bmi_35_39.9: 35-39.9 (condition: BMI 35-39.9)
        - bmi_over_40: >= 40 (condition: BMI > 40)
        - normal_range: 18.5-34.9 (no specific condition trigger in PDF)
    
    Args:
        bmi (float): Body Mass Index value
    
    Returns:
        str: BMI category matching PDF condition names
    """
    if bmi is None:
        return "unknown"
    elif bmi < 18.5:
        return "low_bmi"  # Matches condition "Low BMI < 18.5"
    elif bmi < 35:
        return "normal_range"  # No condition trigger in PDF
    elif bmi < 40:
        return "bmi_35_39.9"  # Matches condition "BMI 35-39.9"
    else:
        return "bmi_over_40"  # Matches condition "BMI > 40"
